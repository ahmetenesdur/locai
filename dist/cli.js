...
import { translateFile, findLocaleFiles, validateAndFixExistingTranslations, } from "./commands/translator.js";
...
const loadConfig = async () => {
    try {
        ...
        try {
            ...
            console.warn(`Warning: Both CommonJS and ES module loading failed: ${importError.message}`);
            throw importError;
        }
    }
    catch (error) {
...
const configureCLI = async (defaultConfig) => {
    ...
    const runCommand = async (options, commandOptions, commandName) => {
        try {
            ...
            if (sanitizedGlobalOpts.source) {
                sanitizedGlobalOpts.source = InputValidator.validateLanguageCode(sanitizedGlobalOpts.source, "source language");
            }
            if (sanitizedGlobalOpts.targets && Array.isArray(sanitizedGlobalOpts.targets)) {
                sanitizedGlobalOpts.targets = InputValidator.validateLanguageCodes(sanitizedGlobalOpts.targets, "target languages");
            }
            if (sanitizedGlobalOpts.localesDir) {
                sanitizedGlobalOpts.localesDir = InputValidator.validateDirectoryPath(sanitizedGlobalOpts.localesDir, "locales directory");
            }
            if (sanitizedCommandOptions.provider) {
                sanitizedCommandOptions.provider = InputValidator.validateProvider(sanitizedCommandOptions.provider, "API provider");
            }
            ...
            if (sanitizedCommandOptions.length) {
                ...
                if (!validLengthModes.includes(sanitizedCommandOptions.length)) {
                    throw new Error(`Invalid length mode: ${sanitizedCommandOptions.length}. Valid modes: ${validLengthModes.join(", ")}`);
                }
            }
            const mergedOpts = {
            ...
            let concurrencyLimit = parseInt(mergedOpts.concurrency) || defaultConfig.concurrencyLimit || 5;
            if (mergedOpts.autoOptimize) {
                ...
                console.log(`Auto-optimized settings for your system (${cpuCount} CPUs, ${memoryGB}GB RAM):`);
            ...
            const finalConfig = {
                ...
                cacheEnabled: mergedOpts.noCache === undefined
                ...
                retryOptions: {
                    ...
                    initialDelay: mergedOpts.initialDelay || defaultConfig.retryOptions?.initialDelay || 1000,
                ...
                context: {
                    ...
                    allowNewCategories: mergedOpts.allowNewCategories !== undefined
                        ? mergedOpts.allowNewCategories
                        : defaultConfig.context.allowNewCategories,
                    detection: {
                        threshold: mergedOpts.contextThreshold ||
                            defaultConfig.context.detection?.threshold ||
                            2,
                        minConfidence: mergedOpts.contextConfidence ||
            ...
            try {
                ...
                const error = ErrorHelper.configValidationError(configError.message.includes("validation failed")
            ...
            if (!files || !files.length) {
                const error = ErrorHelper.fileNotFoundError(`${localesDir}/${finalConfig.source}.json`, `${finalConfig.source}.json`);
                console.error(ErrorHelper.formatError(error, {
            ...
            switch (commandName) {
                case "fix":
                    console.log("\nRunning in FIX mode");
                    await Promise.all(files.map((file) => validateAndFixExistingTranslations(file, finalConfig)));
                    break;
                case "analyze":
                ...
                default:
                    ...
                    if (finalConfig.context.useAI) {
                        console.log(`AI Context Analysis: ENABLED (Provider: ${finalConfig.context.aiProvider || finalConfig.apiProvider})`);
                        if (finalConfig.context.allowNewCategories) {
                            console.log("New category suggestions: ENABLED");
                        }
                    }
                    console.log(`Performance settings:`);
                    console.log(`   - Concurrency: ${finalConfig.concurrencyLimit} parallel operations`);
                    ...
                    if (finalConfig.forceUpdate) {
                        console.log(`Warning: Force update mode ENABLED (will update existing translations)`);
                    }
    ...
    program
        ...
        .option("--concurrency <number>", "Number of concurrent translations", Number, defaultConfig.concurrencyLimit)
        .option("--no-cache", "Disable translation caching")
        .option("--force", "Force update of existing translations", false)
        .option("--length <mode>", "Length control mode", defaultConfig.lengthControl?.mode || "smart")
    ...
    program
        ...
        try {
            ...
            if (error.code && error.code.startsWith("ERR_")) {
                console.error(ErrorHelper.formatError(error, { showDebug: process.env.DEBUG === "true" }));
            }
            else {
                console.error(`\nError: ${error.message}`);
                if (error.stack && process.env.DEBUG) {
    ...
    program
        ...
        .option("--context-provider <provider>", "AI provider for analysis", defaultConfig.context.aiProvider)
        .option("--context-threshold <number>", "Minimum match count", Number, defaultConfig.context.detection.threshold)
        .action(async (options) => {
        try {
    ...
    program
        ...
        .option("--context-confidence <number>", "Minimum confidence score", Number, defaultConfig.context.detection.minConfidence)
        .option("--context-debug", "Show context details", defaultConfig.context.debug)
        .option("--min-text-length <number>", "Minimum text length for AI analysis", Number, defaultConfig.context.minTextLength)
        .option("--allow-new-categories", "Allow AI to suggest new categories", defaultConfig.context.allowNewCategories)
        .option("--max-retries <number>", "Maximum number of retries for API calls", Number, defaultConfig.retryOptions?.maxRetries || 2)
        .action(async (options) => {
        try {
    ...
    program
        ...
        try {
            ...
            try {
                ...
                console.log(`   Targets: ${defaultConfig.targets.length} languages (${defaultConfig.targets.slice(0, 5).join(", ")}${defaultConfig.targets.length > 5 ? "..." : ""})`);
                console.log(`   API Provider: ${defaultConfig.apiProvider || "auto-detect"}`);
                console.log(`   Concurrency: ${defaultConfig.concurrencyLimit || 5} parallel operations`);
                console.log(`   Cache: ${defaultConfig.cacheEnabled !== false ? "Enabled" : "Disabled"}`);
                if (defaultConfig.context?.enabled) {
                    console.log(`   Context Detection: ${defaultConfig.context.useAI ? "AI-powered" : "Keyword-based"}`);
                }
                if (defaultConfig.useFallback) {
                    const fallbackChain = defaultConfig.fallbackOrder?.slice(0, 3).join(" → ") || "auto";
...
const displayPerformanceTips = async (options) => {
    ...
    try {
        ...
        console.log(`   - Cache: ${options.cacheEnabled ? "Enabled" : "Disabled"} (${cacheStats.size} items cached)`);
        // Provide tips
        console.log("\nPerformance Tips:");
        if (cpuCount > options.concurrencyLimit * 2) {
            console.log(`   - Increase concurrency with --concurrency ${Math.min(cpuCount, 10)} to better utilize your CPU`);
        }
        if (!options.cacheEnabled) {
            console.log(`   - Enable caching to improve repeated runs speed`);
        }
        if (memoryGB < 4) {
            console.log(`   - Your system has limited memory (${memoryGB}GB). Consider reducing concurrency if you experience issues.`);
        }
    }
    catch (error) {
