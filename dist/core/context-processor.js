...
class ContextProcessor {
    ...
    async analyzeBatch(texts) {
        ...
        if (aiTexts.length > 0 && this.config.useAI) {
            try {
                const aiResults = await this.aiAnalyzer.analyzeBatch(aiTexts, this.config.aiProvider || "openai");
                ...
                if (validAiResults.length > 0) {
                    log(`Batch AI Context Analysis: ${validAiResults.length}/${aiTexts.length} analyzed`, true);
                }
            }
            catch (error) {
