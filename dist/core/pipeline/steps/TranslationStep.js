...
class TranslationStep extends PipelineStep {
    ...
    async execute(context, next) {
        try {
            ...
            if (this.confidenceSettings.enabled &&
                ...
                const rawResponse = await provider.translate(context.protectedText || context.sourceText, context.sourceLang, context.targetLang, { ...translationOpts, returnRawResponse: true });
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                const result = provider.extractTranslationWithConfidence(rawResponse, 
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                provider.name || "unknown", context.protectedText || context.sourceText, context.sourceLang, context.targetLang, context.meta?.category);
                context.translatedText = result.translation;
                context.confidence = result.confidence;
            }
            else {
                // Standard translation
                context.translatedText = await provider.translate(context.protectedText || context.sourceText, context.sourceLang, context.targetLang, translationOpts);
    ...
    _getProvider() {
        const provider = ProviderFactory.getProvider(this.options.apiProvider, this.options.useFallback !== false, this.options);
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        if (!provider || typeof provider.translate !== "function") {
            throw new Error(`Translation provider not available or invalid: ${this.options.apiProvider}`);
        }
        return provider;
    }
}
