...
class TranslationService {
    ...
    async translateFile(file, options) {
        ...
        try {
            ...
            const { stateManager, projectRoot, currentState, comparison } = await this.initializeTranslationState(resolvedFile, flattenedSource, options);
            ...
            const globalStats = statisticsManager.getStats();
            try {
                await this.processAllLanguages(resolvedFile, flattenedSource, options, globalStats, comparison);
                await this.finalizeTranslation(stateManager, projectRoot, currentState, globalStats, startTime, options);
                return globalStats;
            }
            catch (error) {
     ...
     */
    async validateTranslationInputs(file, options) {
        ...
        if (options.localesDir) {
            options.localesDir = InputValidator.validateDirectoryPath(options.localesDir, "locales directory");
        }
     ...
     */
    async initializeTranslationState(resolvedFile, flattenedSource, options) {
        ...
        if (stats.hasChanges) {
            ...
            if (comparison?.deletedKeys &&
                ...
                await this.removeDeletedKeysFromTargets(resolvedFile, comparison.deletedKeys, options);
            }
        }
     ...
     */
    async processAllLanguages(resolvedFile, flattenedSource, options, globalStats, comparison) {
        ...
        for (let i = 0; i < targetLanguages.length; i += languageConcurrency) {
            ...
            const batchResults = await Promise.all(currentBatch.map((targetLang) => {
                ...
                return this.processLanguage(targetLang, resolvedFile, flattenedSource, orchestrator, { ...options, progressOptions }, globalStats, comparison);
            }));
     ...
     */
    async processLanguage(targetLang, sourceFile, flattenedSource, orchestrator, options, globalStats, comparison) {
        ...
        try {
            ...
            try {
                ...
                if (err.code === "ENOENT") {
                    await uiManager.log(`\ud83c\udd95 Creating new translation file for ${safeTargetLang}`);
                }
                else {
                    if (options.debug) {
            ...
            for (const [key, sourceText] of Object.entries(flattenedSource)) {
                try {
                    ...
                    await uiManager.log(`\u26a0\ufe0f Skipping invalid key/text: ${keyError.message}`);
                ...
                if (this.isPlaceholderOnlyText(sourceText)) {
                    ...
                    if (existingValue !== sourceText) {
                        ...
                        if (options.debug) {
                            await uiManager.log(`   Copying placeholder-only text: "${sourceText}"`);
                        }
                    }
            ...
            if (missingKeys.length > 0) {
                await uiManager.log(`Found ${missingKeys.length} missing translations for ${safeTargetLang}`);
            }
            let results = [];
            if (missingKeys.length > 0) {
                try {
            ...
            try {
                ...
                validResults = validResults.filter((result) => result.key &&
     ...
     * @param {TranslationOptions} options - Translation options.
     */
    async finalizeTranslation(stateManager, projectRoot, currentState, globalStats, startTime, options) {
        ...
        if (options.saveReviewQueue || options.minConfidence !== undefined) {
            if (globalStats.orchestrators && globalStats.orchestrators.length > 0) {
                ...
                globalStats.orchestrators.forEach((orchestrator) => {
                    if (orchestrator.confidenceSettings &&
     ...
     * @param {TranslationOptions} options - Translation options.
     */
    async removeDeletedKeysFromTargets(sourceFile, deletedKeys, options) {
        const sourceDir = path.dirname(sourceFile);
        let totalRemoved = 0;
        ...
        for (const targetLang of options.targets || []) {
            try {
                ...
                if (removedFromThisFile > 0) {
                    const unflattened = ObjectTransformer.unflatten(flattenedTarget);
                    await FileManager.writeJSON(targetPath, unflattened);
                    await uiManager.log(`   ${targetLang}.json: Removed ${removedFromThisFile} keys`);
                }
                else {
                ...
                await uiManager.log(`   \u274c Error processing ${targetLang}.json: ${error.message}`);
            }
        }
        await uiManager.log(`Cleanup Summary: Removed ${totalRemoved} keys from ${filesProcessed} files\n`);
    }
    /**
     ...
     */
    async validateAndFixExistingTranslations(file, options) {
        await uiManager.log(`\nChecking existing translations in: "${path.basename(file)}"`);
        const sourceContent = await FileManager.readJSON(file);
        const flattenedSource = ObjectTransformer.flatten(sourceContent);
        const orchestrator = new Orchestrator(options);
        const languageResults = await Promise.all((options.targets || []).map(async (targetLang) => {
            try {
        ...
        try {
            ...
            for (const langData of validLanguages) {
                ...
                if (invalidItems.length > 0) {
                    ...
                    await uiManager.log(`Found ${invalidItems.length} length issues in ${targetLang}`);
                    ...
                    const unflattened = ObjectTransformer.unflatten(flattenedTarget);
                    await FileManager.writeJSON(targetPath, unflattened);
                    await uiManager.log(`Fixed ${fixedCount}/${invalidItems.length} translations in ${targetLang}`);
                }
                else {
     ...
     * Find locale files
     */
    async findLocaleFiles(localesDir, sourceLang) {
        try {
