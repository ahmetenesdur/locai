...
class AIContextAnalyzer {
        ...
        if (config.categories) {
            Object.entries(config.categories).forEach(([category, data]) => {
                if (data.keywords && Array.isArray(data.keywords)) {
                    this.categoryKeywords[category] = new Set(data.keywords.map((k) => k.toLowerCase()));
                }
            });
        }
        this.stats = {
    ...
    async _batchAIAnalysis(texts, apiProvider) {
        ...
        const result = await rateLimiter.enqueue(apiProvider.toLowerCase(), () => provider.analyze(batchPrompt, {
