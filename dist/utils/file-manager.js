                ...
                try {
                    ...
                    if (tempFileCreated) {
                        try {
                            ...
                            console.warn(`Warning: Failed to clean up temporary file ${tempFile}: ${cleanupError.message}`);
                        }
                    }
                    // Re-throw the original error with better context
                    throw new Error(`Atomic write failed during rename operation (${filePath}): ${renameError.message}. ` +
...
class SyncFileManager {
     ...
     */
    static configure(options) {
        console.warn("DEPRECATION WARNING: SyncFileManager is deprecated. Use async FileManager for better performance.");
        if (!options)
            return;
        this.options = {
            ...this.defaultOptions,
    ...
    static findLocaleFiles(localesDir, sourceLang) {
        console.warn("DEPRECATION WARNING: SyncFileManager.findLocaleFiles() is deprecated. Use async FileManager.findLocaleFiles() for better performance.");
        const sourceFile = path.join(localesDir, `${sourceLang}.json`);
    ...
    static readJSON(filePath, options = {}) {
        console.warn("DEPRECATION WARNING: SyncFileManager.readJSON() is deprecated. Use async FileManager.readJSON() for better performance.");
        const config = { ...this.getConfig(), ...options };
        try {
    ...
    static writeJSON(filePath, data, options = {}) {
        console.warn("DEPRECATION WARNING: SyncFileManager.writeJSON() is deprecated. Use async FileManager.writeJSON() for better performance.");
        const config = { ...this.getConfig(), ...options };
        try {
            // Create target directory if it doesn't exist
            const dir = path.dirname(filePath);
    ...
    static deleteFile(filePath, options = {}) {
        console.warn("DEPRECATION WARNING: SyncFileManager.deleteFile() is deprecated. Use async FileManager.deleteFile() for better performance.");
        const config = { ...this.getConfig(), ...options };
        try {
            // Create backup before deletion if backups are enabled
            if (config.backupFiles && config.backupDir && fsSync.existsSync(filePath)) {
