...
class InputValidator {
    ...
    static validateLanguageCode(langCode, paramName = "language") {
        ...
        if (!this.LANGUAGE_CODE_PATTERN.test(sanitized)) {
            throw new Error(`Invalid ${paramName} code: '${langCode}'. Must match pattern: ${this.LANGUAGE_CODE_PATTERN}`);
        }
        return sanitized;
    }
    /**
    ...
    static validateLanguageCodes(langCodes, paramName = "languages") {
        ...
        return langCodes.map((code, index) => this.validateLanguageCode(code, `${paramName}[${index}]`));
    }
    /**
    ...
    static validateDirectoryPath(dirPath, paramName = "directory") {
        ...
        if (!resolved.startsWith(cwd)) {
            throw new Error(`${paramName} path '${dirPath}' is outside working directory (resolved: ${resolved})`);
        }
        return resolved;
    }
    /**
    ...
    static validateText(text, paramName = "text") {
        ...
        if (text.length > this.MAX_TEXT_LENGTH) {
            throw new Error(`${paramName} too long (${text.length} chars, max ${this.MAX_TEXT_LENGTH})`);
        }
        return text;
    }
    /**
    ...
    static validateKey(key, paramName = "key") {
        ...
        if (key.length > this.MAX_KEY_LENGTH) {
            throw new Error(`${paramName} too long (${key.length} chars, max ${this.MAX_KEY_LENGTH})`);
        }
    ...
    static validateProvider(provider, fieldName = "API provider") {
        ...
        if (!this.VALID_PROVIDERS.includes(normalized)) {
            throw new Error(`Invalid ${fieldName}: '${provider}'. Valid providers: ${this.VALID_PROVIDERS.join(", ")}`);
        }
        return normalized;
    }
    /**
    ...
    static validateApiConfig(apiConfig, errors, warnings) {
        for (const [provider, settings] of Object.entries(apiConfig)) {
            ...
            if (settings.temperature !== undefined) {
                ...
                if (temp > 0.5) {
                    warnings.push(`apiConfig.${provider}.temperature is high (${temp}), may reduce consistency`);
                }
            }
            // Validate maxTokens
     ...
     * @param {string[]} warnings - Array to push warnings to.
     */
    static validateRateLimiterConfig(rateLimiter, errors, _warnings) {
        if (rateLimiter.queueStrategy) {
            const validStrategies = ["fifo", "priority"];
            if (!validStrategies.includes(rateLimiter.queueStrategy)) {
                errors.push(`rateLimiter.queueStrategy must be one of: ${validStrategies.join(", ")}`);
            }
        }
                ...
                for (const [provider, limits] of Object.entries(rateLimiter.providerLimits)) {
                    ...
                    if (limits.rpm !== undefined) {
                        ...
                        if (isNaN(rpm) || rpm < 1 || rpm > 10000) {
                            errors.push(`rateLimiter.providerLimits.${provider}.rpm must be between 1 and 10000`);
                        }
                    }
                    if (limits.concurrency !== undefined) {
                        const conc = parseInt(limits.concurrency);
                        if (isNaN(conc) || conc < 1 || conc > 50) {
                            errors.push(`rateLimiter.providerLimits.${provider}.concurrency must be between 1 and 50`);
                        }
                    }
                }
            }
