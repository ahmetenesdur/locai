...
class RateLimiter {
    ...
    _adjustThrottling() {
        ...
        Object.keys(this.providers).forEach((provider) => {
            ...
            const avgResponseTime = respTimes.length === 0
                ? 0
            ...
            if (concurrencyAdjustment !== 0) {
                const newConcurrency = Math.max(1, this.providers[provider].maxConcurrent + concurrencyAdjustment);
                this.providers[provider].maxConcurrent = newConcurrency;
                this.metrics.adjustments[provider].concurrency += concurrencyAdjustment;
            }
            if (rpmAdjustment !== 0) {
                const newRPM = Math.max(10, this.providers[provider].requestsPerMinute + rpmAdjustment);
    ...
    clearAllQueues() {
        for (const provider in this.queues) {
            if (this.queues[provider] && this.queues[provider].length > 0) {
                console.log(`Clearing ${this.queues[provider].length} pending operations for ${provider}`);
