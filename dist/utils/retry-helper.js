...
class RetryHelper {
            ...
            try {
                ...
                if (attempts > 0) {
                    ...
                    if (process.env.DEBUG) {
                        ...
                        console.log(`${context}${providerInfo}: Retrying attempt ${attempts}/${maxRetries} after ${delay}ms delay`);
                    }
                ...
                if (process.env.DEBUG) {
                    const providerInfo = providerName ? ` [${providerName}]` : "";
                    console.warn(`Warning: ${context}${providerInfo}: Error on attempt ${attempts}/${maxRetries + 1}: ${error.message}` +
    ...
    static defaultRetryCondition(error, retryableErrors = []) {
        ...
        if (retryableErrors.includes("rate_limit") &&
            (error.status === 429 || error.code === "ERR_RATE_LIMIT")) {
            return true;
        }
        // Check for timeout errors
        if (retryableErrors.includes("timeout") &&
