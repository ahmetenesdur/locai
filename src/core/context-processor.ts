...
class ContextProcessor {
	...
	private keywordCache: Map<string, KeywordConfig>;
	private aiAnalyzer: AIContextAnalyzer;
	// eslint-disable-next-line @typescript-eslint/no-explicit-any
	private resultCache: LRUCache<string, any> | null;
	 ...
	 * @returns - Array of context analysis results.
	 */
	async analyzeBatch(texts: string[]): Promise<AnalysisResult[]> {
		...
		for (let i = 0; i < texts.length; i++) {
			...
			if (
				...
				!this.config.useAI ||
				(this.config.minTextLength && text.length < this.config.minTextLength)
			) {
		...
		if (aiTexts.length > 0 && this.config.useAI) {
			try {
				...
				const validAiResults = aiResults.filter((r) => r);
				if (validAiResults.length > 0) {
					log(
						`Batch AI Context Analysis: ${validAiResults.length}/${aiTexts.length} analyzed`,
						true
					);
				}
			} catch (error: any) {
	 ...
	 */
	private _performKeywordAnalysis(text: string): AnalysisResult {
	 ...
	 */
	// eslint-disable-next-line @typescript-eslint/no-explicit-any
	private getBestMatch(results: Map<string, any>, totalScore: number): AnalysisResult {
	 ...
	 */
	private getFallback(): AnalysisResult {
		return {
