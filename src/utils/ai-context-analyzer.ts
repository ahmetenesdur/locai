...
// Define interfaces for options and results
...
class AIContextAnalyzer {
	...
	async analyzeBatch(
		...
		for (let i = 0; i < texts.length; i++) {
			...
			if (text.length < (this.config.minTextLength || 10)) {
			...
			if (this.cache.has(cacheKey)) {
				...
				results[i] = this.cache.get(cacheKey) || null;
				continue;
			}
	...
	async _batchAIAnalysis(
		...
		const provider = ProviderFactory.getProvider(apiProvider, true, this.config as any);
		if (!provider) {
		...
		const result = await rateLimiter.enqueue(apiProvider.toLowerCase(), () =>
			(provider as any).analyze(batchPrompt, {
		...
		return this._parseBatchAnalysisResult(result as string, texts.length);
	}

	// Create batch analysis prompt for multiple texts
	_createBatchAnalysisPrompt(texts: string[]): string {
...
`;
	...
	_createAnalysisPrompt(text: string): string {
		const categories = Object.keys(this.config.categories || {}).join(", ");
...
`;
	...
	_createSimplifiedAnalysisPrompt(text: string): string {
		const categories = Object.keys(this.config.categories || {}).join(", ");
...
`;
	...
	_parseBatchAnalysisResult(result: string, expectedCount: number): (AnalysisResult | null)[] {
		try {
			...
			for (const item of analysisArray) {
				...
				if (
					...
					this.config.allowNewCategories
				) {
					this._saveNewCategory(category, item.keywords || []);
					this.stats.newCategories++;
				} else if (this.config.categories && !this.config.categories[category]) {
				...
				results[index] = {
					...
					prompt:
						this.config.categories?.[category]?.prompt || this.config.fallback?.prompt,
					method: "batch_ai",
				};
			}
	...
	_parseAnalysisResult(result: string): AnalysisResult | null {
		try {
			const jsonMatch = result.match(/\{[\s\S]*?\}/);
			if (!jsonMatch) {
	...
	_fastKeywordMatch(text: string): AnalysisResult | null {
		...
		if (this.config.detection && confidence < (this.config.detection.minConfidence || 0.4)) {
			return this._getDefaultContext();
		}
	...
	_getDefaultContext(): AnalysisResult {
		return {
	...
	_findClosestCategory(suggestedCategory: string, keywords: string[]): string {
		let bestMatch = this.config.fallback?.category || "general";
		let highestScore = 0;

		if (!this.config.categories) return bestMatch;
	...
	_saveNewCategory(category: string, keywords: string[]): void {
