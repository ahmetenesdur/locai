...
class ConfidenceScorer {
	 ...
	 * @param {number} options.aiConfidence - AI provider's confidence (0-1)
	 * @param {string} options.sourceText - Original text
	 ...
	 * @returns {Object} { score, details, issues }
	 */
	...
	static extractAIConfidence(response, provider) {
		...
		if (provider === "openai" && response.choices?.[0]?.logprobs) {
			...
			if (logprobs.length > 0) {
				...
				const avgLogProb =
					logprobs.reduce((sum, lp) => sum + (lp || -1), 0) / logprobs.length;
