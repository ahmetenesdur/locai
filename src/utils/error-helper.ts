...
export interface HelperError extends Error {
	code?: string;
	type?: string;
...
class ErrorHelper {
	 ...
	 */
	static createError(type: string, details: ErrorDetails = {}): HelperError {
		const errorInfo = this._getErrorInfo(type, details);
		const error: HelperError = new Error(errorInfo.message);
	...
	static formatError(
		error: HelperError,
		options: { showDebug?: boolean; showSolutions?: boolean; showContext?: boolean } = {}
	): string {
	 ...
	 */
	private static _getErrorInfo(
		type: string,
		details: ErrorDetails
	): { code: string; message: string; context: string[]; solutions: string[] } {
		...
		> = {
			...
			CONFIG_VALIDATION: {
				...
				message: details.validationErrors
					? `Configuration has ${details.validationErrors.length} error(s):\n${details.validationErrors.map((e: string, i: number) => `   ${i + 1}. ${e}`).join("\n")}`
	 ...
	 */
	private static _buildRateLimitMessage(details: ErrorDetails): string {
	 ...
	 */
	private static _getEnvKeyName(provider: string): string {
	 ...
	 */
	private static _getProviderConsoleUrl(provider: string): string {
	 ...
	 */
	static rateLimitError(provider: string, details: ErrorDetails = {}): HelperError {
	...
	static networkError(provider: string): HelperError {
		return this.createError("API_NETWORK", { provider });
	}

	static timeoutError(provider: string, timeout?: number): HelperError {
		return this.createError("API_TIMEOUT", { provider, timeout });
	}

	static configNotFoundError(): HelperError {
		return this.createError("CONFIG_NOT_FOUND");
	}

	static configValidationError(validationErrors: string[]): HelperError {
		return this.createError("CONFIG_VALIDATION", { validationErrors });
	}

	static fileNotFoundError(filePath: string, expectedSource?: string): HelperError {
		return this.createError("FILE_NOT_FOUND", { filePath, expectedSource });
	}

	static translationError(key: string, language: string, reason?: string): HelperError {
		return this.createError("TRANSLATION_FAILED", { key, language, reason });
	}

	static languageValidationError(langCode: string, field?: string): HelperError {
