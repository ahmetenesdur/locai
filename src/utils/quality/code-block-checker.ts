export interface CodeBlockIssue {
	type: "code-block-count" | "code-block-modified" | "code-block-restore";
	severity: "error" | "critical";
	message: string;
	details?: any;
}

export interface CodeBlockFixResult {
	text: string;
	foundIssues: CodeBlockIssue[];
	appliedFixes: { type: "code-block-restore"; message: string }[];
}

export interface CodeBlock {
	type: "fenced" | "inline" | "html-code" | "html-pre";
	full: string;
	code: string;
	position: number;
}
...
class CodeBlockChecker {
	 ...
	 */
	checkCodeBlockPreservation(sourceText: string, translatedText: string): CodeBlockIssue[] {
	 ...
	 */
	fixCodeBlockPreservation(sourceText: string, translatedText: string): CodeBlockFixResult {
		const foundIssues: CodeBlockIssue[] = [];
		const appliedFixes: { type: "code-block-restore"; message: string }[] = [];
	 ...
	 */
	protectCodeBlocks(text: string): { protectedText: string; tokenMap: Map<string, string> } {
	 ...
	 */
	restoreCodeBlocksFromTokens(text: string, tokenMap: Map<string, string>): string {
