export interface MarkdownIssue {
	type: "markdown-preservation" | "markdown-link" | "markdown-missing";
...
export interface MarkdownFixResult {
	text: string;
	foundIssues: MarkdownIssue[];
	appliedFixes: { type: "markdown-link"; message: string }[];
}
...
class MarkdownChecker {
	private markdownPatterns: Record<string, RegExp>;
	 ...
	 */
	checkMarkdownPreservation(sourceText: string, translatedText: string): MarkdownIssue[] {
	 ...
	 * Check for broken markdown links.
	 */
	checkBrokenMarkdownLinks(text: string): MarkdownIssue[] {
		const issues: MarkdownIssue[] = [];
		const linkPattern = /\[([^\]]+)\]\(([^)]+)\)/g;
	 ...
	 */
	fixMarkdownPreservation(sourceText: string, translatedText: string): MarkdownFixResult {
		let fixedText = translatedText;
		const foundIssues: MarkdownIssue[] = [];
		const appliedFixes: { type: "markdown-link"; message: string }[] = [];
		...
		for (const [type, elements] of Object.entries(sourceMarkdown)) {
			if (elements.length > 0) {
				...
				// fixes are not really applied in restoreMarkdownElements for now
			}
		}
	 ...
	 */
	extractMarkdownElements(text: string): Record<string, any[]> {
	 ...
	 */
	restoreMarkdownElements(
		text: string,
		elements: any[],
		type: string
	): { text: string; foundIssues: MarkdownIssue[]; appliedFixes: any[] } {
		const foundIssues: MarkdownIssue[] = [];
