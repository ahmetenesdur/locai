...
export interface PlaceholderFixResult {
	...
	appliedFixes: { type: "placeholder"; message: string }[];
}
...
class PlaceholderChecker {
	...
	fixPlaceholders(source: string, translated: string): PlaceholderFixResult {
		...
		const appliedFixes: { type: "placeholder"; message: string }[] = [];
	 ...
	 */
	findCorruptedPlaceholder(translated: string, sourcePlaceholder: string): string | null {
		...
		const corruptionPatterns = [
			...
			new RegExp(
				`\\{[^}]{${Math.max(1, placeholderName.length - 2)},${
	 ...
	 */
	findBestPlaceholderPosition(translated: string, source: string, placeholder: string): number {
	 ...
	 */
	insertPlaceholder(text: string, placeholder: string, position: number): string {
