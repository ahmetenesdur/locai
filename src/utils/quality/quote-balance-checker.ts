export interface QuoteIssue {
	type: "unbalanced-quotes" | "quote-consistency" | "quote-balance";
	severity: "error" | "warning";
	message: string;
	char?: string;
}

export interface QuoteFixResult {
	text: string;
	foundIssues: QuoteIssue[];
	appliedFixes: { type: "quote-balance"; message: string }[];
}
...
class QuoteBalanceChecker {
	 ...
	 */
	checkQuoteBalance(text: string): QuoteIssue[] {
	 ...
	 */
	private _countQuotes(text: string, quoteChar: string): { count: number; unbalanced: boolean } {
	 ...
	 */
	fixQuoteBalance(text: string): QuoteFixResult {
		let fixedText = text;
		const foundIssues: QuoteIssue[] = [];
		const appliedFixes: { type: "quote-balance"; message: string }[] = [];
	 ...
	 */
	validateQuoteConsistency(source: string, translated: string): QuoteIssue[] {
		const issues: QuoteIssue[] = [];
