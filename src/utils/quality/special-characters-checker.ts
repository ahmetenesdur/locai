...
export interface SpecialCharFixResult {
	text: string;
	foundIssues: SpecialCharIssue[];
	appliedFixes: {
		type: "critical-char-restore" | "character-substitution";
		message: string;
	}[];
}
...
class SpecialCharactersChecker {
	private specialCharacterSets: Record<string, RegExp>;
	private criticalCharacters: string[];
	 ...
	 */
	checkSpecialCharacters(sourceText: string, translatedText: string): SpecialCharIssue[] {
		...
		for (const [category, pattern] of Object.entries(this.specialCharacterSets)) {
			const sourceMatches: string[] = sourceText.match(pattern) || [];
			const translatedMatches: string[] = translatedText.match(pattern) || [];

			if (sourceMatches.length !== translatedMatches.length) {
				issues.push({
					type: "special-characters",
					category,
					severity: this.criticalCharacters.some((char: string) =>
						sourceMatches.includes(char)
					)
						? "error"
						: "warning",
					...
					details: {
						...
						missing: sourceMatches.filter(
							(char: string) => !translatedMatches.includes(char)
						),
						extra: translatedMatches.filter(
							(char: string) => !sourceMatches.includes(char)
						),
					},
				});
			}
	 ...
	 */
	checkEmojiPreservation(sourceText: string, translatedText: string): SpecialCharIssue[] {
		const issues: SpecialCharIssue[] = [];
	 ...
	 */
	fixSpecialCharacters(sourceText: string, translatedText: string): SpecialCharFixResult {
		let fixedText = translatedText;
		const foundIssues: SpecialCharIssue[] = [];
		const appliedFixes: {
			type: "critical-char-restore" | "character-substitution";
			message: string;
		}[] = [];
	 ...
	 */
	extractSpecialCharacters(text: string): Record<string, string[]> {
	 ...
	 */
	restoreCriticalCharacters(
		sourceText: string,
		translatedText: string,
		missingChars: string[]
	): {
		text: string;
		appliedFixes: { type: "critical-char-restore"; message: string }[];
	} {
		const appliedFixes: { type: "critical-char-restore"; message: string }[] = [];
	 ...
	 * Fix common character substitutions.
	 */
	fixCommonSubstitutions(
		sourceText: string,
		translatedText: string
	): {
		text: string;
		foundIssues: SpecialCharIssue[];
		appliedFixes: { type: "character-substitution"; message: string }[];
	} {
		const foundIssues: SpecialCharIssue[] = [];
		const appliedFixes: { type: "character-substitution"; message: string }[] = [];
		let fixedText = translatedText;
