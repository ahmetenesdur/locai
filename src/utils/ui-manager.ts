import { GlobalStats } from "./statistics-manager.js";
...
export class UIManager {
	 ...
	 */
	async displayGlobalSummary(
		stats: GlobalStats & { totalDuration?: number },
		...
		await this.log(
			`Average per language: ${(stats.totalTime / (totalLanguages || 1)).toFixed(1)}s`
		);
		...
		if (Object.keys(stats.languages).length > 0) {
			...
			for (const [lang, langStats] of Object.entries(stats.languages)) {
				...
				await this.log(
					`  ${lang.padEnd(4)} | ${String(langStats.added).padStart(3)} added | ${String(
						langStats.skipped
					).padStart(3)} skipped | ${String(langStats.failed).padStart(
						2
					)} failed | ${timeSeconds.toFixed(1)}s`
				);
			}
		}
	...
	async logBatchResults(batchResults: any[]): Promise<void> {
